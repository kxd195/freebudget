...
@section('scripts')
<script type="text/javascript" src="{{ asset('js/shows.edit.js') }}"></script>
@endsection

@section('content')
<div class="small">
	<a href="{{ route('home') }}"><strong>Home</strong></a>
	&gt; {{ $entry->name or 'Create a New Show' }} 
</div>
<div class="panel panel-primary">
    <div class="panel-heading"><h1 class="panel-title">{{ $entry->name or 'Create a New Show' }}</h1></div>
    <div class="panel-body">
	{{ Form::model($entry, [
		'route' => isset($entry->id) ? ['shows.update', $entry->id] : 'shows.store', 
	...
	<div class="form-group @if ($errors->has('type')) has-error @endif">
		...
		<div class="col-md-9">
		{{ Form::select('type', App\Show::getTypes(), $entry->type, ['class' => 'form-control', 'placeholder' => 'Select...', 'onchange' => 'toggleType()']) }}
	...
	<div class="form-group @if ($errors->has('qty')) has-error @endif" id="pane-qty">
		...
		<div class="col-md-3">
		{{ Form::number('qty', $entry->qty, ['class' => 'form-control', 'step' => 'any', 'min' => 0]) }}
		</div>
		@if ($errors->has('qty')) <p class="help-block">{{ $errors->first('qty') }}</p> @endif
	</div>

	<div class="form-group">
		{{ Form::label('', 'Working Days:', ['class' => 'col-md-3 control-label']) }}
	...
	<div class="form-group @if ($errors->has('num_union')) has-error @endif">
		{{ Form::label('num_union', 'Min. Union Personnel:', ['class' => 'col-md-3 control-label']) }}
		<div class="col-md-2">
		{{ Form::number('num_union', $value = null, ['class' => 'form-control', 'step' => 'any', 'min' => 0]) }}
		</div>
	...
	@component('components.delete', ['entry' => $entry, 'controller' => 'shows', 'caption' => 'Delete Show'])
	@endcomponent
	...
	<div class="form-group">
		...
		<ul>
			...
			@endforeach
			<li><a href="{{ route('budgets.create', ['show_id' => $entry->id]) }}"><em>Create a New Budget</em></a></li>
		</ul>
		</div>
	</div>
	</div>
